...
public class Node extends BaseEntity {
	...
	public void draw(Graphics2D g, double interpolate, QuadTree qtree, int noNodes) {
		super.draw(g, interpolate);
		
		ArrayList<Point> points = qtree.query(new Circle(
			((int) pos.x), ((int) pos.y), 200)
		);
		...
		for (int i = 0; i < points.size(); i++) {
			...
			g.setColor(
				ColorMaterial.withAlpha(
					c,
					(int) Utility.map(dist, 0, 200, 63, 0)
				)
			);
			g.drawLine(
		...
		g.setColor(
			ColorMaterial.withAlpha(
				c,
				(int) Utility.map(points.size(), 0, noNodes, 100, 200)
			)
		);
		
		g.fillOval(
			(int) drawPos.x - (width / 2),
			(int) drawPos.y - (height / 2),
			width,
			height
		);
